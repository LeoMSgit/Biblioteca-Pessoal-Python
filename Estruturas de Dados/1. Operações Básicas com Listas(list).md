...
- A Compreensão de Listas é um modo mais prático, porém avançado de criar loops para filtragem em listas
- A sintaxe básica é:
```python
[expressão for item in iterável if condição]
```
Onde:
expressão → O valor que será adicionado à lista
for item in iterável → Iteração sobre uma sequência (lista, tupla, range, etc.)
if condição (opcional) → Filtro para selecionar elementos
