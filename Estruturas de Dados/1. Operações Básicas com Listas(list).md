...
Aqui temos: `expressão → [letra.upper() for letra in palavra]`, significando que todas as letras da palavra serão converidas para maiúsculas e adicionadas em uma nova lista nova lista interna <br/>
...
# ['Par: 0', 'Par: 2', 'Par: 4', 'Par: 6', 'Par: 8']
```
Aqui temos: `expressão → f"Par: {x}"`, significando que a lista será modificada com a adição 'Par: ', antes do valor de x <br/>
for item in `iterável → for x in range(10)`, delimitando o tamanho da sequência <br/>
if condição → `if x % 2 == 0`, se o elemento iterado aceitar essa condição, ele será adicionado na lista, realizando uma filtragem <br/>

5.3) Loop Condicional If-else (Condição Ternária)

Aqui temos: `expressão → "par" if x % 2 == 0 else "ímpar"`, significando que a lista será modificada, trocando os valores de x por "par" ou "impar" dependendo do número em questão <br/>
